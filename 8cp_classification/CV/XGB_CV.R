...
levels(input$genotype) <- c(0,1) #1 is WT, 0 is Q344X
input$genotype <- as.numeric(as.character(input$genotype))

#one data, loop through feature sets and hyperparameters
cl <- makeCluster(2)#doParallel, setup parallel processing
registerDoParallel(cl)
CV_list <- foreach(i = 1:length(fold_uid_list), .packages = c("xgboost", "pROC", "caret")) %dopar% {
  ...
  score_df <- as.data.frame(XGB_acc(train_fold[,-1], valid_fold[,-1], XGB_parameters))
