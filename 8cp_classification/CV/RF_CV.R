...
CV_list <- foreach(i = 1:length(fold_uid_list), .packages = c("randomForest", "pROC", "caret")) %dopar% {
  ...
  score_df <- as.data.frame(RF_k_acc(train_fold[,-1], valid_fold[-1], RF_parameters))
