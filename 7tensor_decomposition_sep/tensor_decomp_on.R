...
output_folder <- "7tensor_decomposition_sep/output_on/"
dir.create(output_folder)#create the output folder if not existing
#import data
on_data <- read.csv('2preprocess_sep/Opt_on_train_norm.csv')
#sort by genotype and time
on_data <- on_data[order(on_data$genotype, on_data$start, on_data$location),]
on_data <- on_data[on_data$start >= 0, ]
#transform long to 3d array
on_list <- split(on_data, f = as.factor(on_data$start))
#keep only start and functional variables
on_list_sub <- lapply(on_list, function(x) as.matrix(x[,6:14]))
on_array <- simplify2array(on_list_sub)

#select the best rank
cl <- makeCluster(6)#doParallel, setup parallel processing
registerDoParallel(cl)
rank_list_cp <- foreach(i = 1:50, .packages = c("rTensor")) %dopar% {cp(as.tensor(on_array), num_components = i, max_iter = 100)}
rank_error <- unlist(lapply(rank_list_cp, function(x) x$fnorm_resid))
pdf(paste(output_folder, "error_plot_on.pdf"))
plot(rank_error, type = "b")
dev.off()

#use rank 10
cp_decomp_opt <- cp(as.tensor(on_array), num_components = 3, max_iter = 500)
rank_num <- 1:3
#plot by samples
...
behavior_plot <- function(cp_obj, n_rank)
{
  ...
  plot_dat$behavior <- colnames(on_data)[6:14]
