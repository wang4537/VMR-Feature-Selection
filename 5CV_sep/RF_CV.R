...
RF_k_acc <- function(train, valid, RF_parm)
{
  auc_score <- apply(RF_parm, 1, function(i)
  {
    #build the svm classifier
    classifier <- randomForest(uid~.,
                               data = train,
                               proximity = TRUE,
                               ntree = i[1],
                               mtry = i[2])
...
MAT_CV <- function(i) # i = matricization method name
{
  ...
  ave_cv_feature.list <- lapply(feature_set, function(y) #loop through feature sets
  {
    ...
    CV_list <- lapply(fold.list, function(k) #loop through folds
    {
      ...
      #replace the uid by genotype in train and valid
      train_fold$uid <- sapply(strsplit(train_fold$uid, "_"), '[', 3)
      train_fold$uid <- as.factor(as.numeric(factor(train_fold$uid)))
      valid_fold$uid <- sapply(strsplit(valid_fold$uid, "_"), '[', 3)
      valid_fold$uid <- as.factor(as.numeric(factor(valid_fold$uid)))

      #calculate the CV accuracy
      score_df <- as.data.frame(RF_k_acc(train_fold, valid_fold, RF_parameters))
      return (score_df) #return the mean accuracy of 10 folds in each parameter
    }) # multicore processing

    #as.data.frame(CV_list)
    ave_CV_df <- cbind(RF_parameters, 'AUROC' = as.numeric(rowMeans(as.data.frame(CV_list)))) #add HPs to the df
...
setwd("/depot/yleung/data/Feature_selection_framework/5CV_sep/")
#create the output directories
class_name <- 'RF'
dir.create(class_name)
#list of hyperparameters
RF_parameters <- expand.grid(ntree = c(125,250,500,1000),
                             mtry = c(8,16,32,64,128,256))
...
mat_cv_list <- foreach(i = 1:length(mat_names), .packages = c("randomForest", "pROC", "caret")) %dopar% {MAT_CV(mat_names[i])}
...
mat_names <- gsub("../3transformed_data_sep/|/.*$", "", off_files) #keep offly the mid dir name
input_list <- lapply(off_files, read.csv) #read the files
names(input_list) <- mat_names #rename the list

#### FILTER #####
#read filter lists
off_filter_files <- list.files("../4feature_selection_sep/filter_wilc", pattern = "*off_vol.csv", full.names = TRUE, recursive = TRUE)
...
#### EMBEDDED ####
#read embedded lists
off_embedded_files <- list.files("../4feature_selection_sep/embedded_wilc/", pattern = "*off_sigVar_pass_list.csv", full.names = TRUE, recursive = TRUE)
