...
kNN_k_acc <- function(train, valid, knn_k)
{
  auc_score <- sapply(knn_k, function(i)
  {
...
MAT_CV <- function(i) # i = matricization method name
{
  ...
  ave_cv_feature.list <- lapply(feature_set, function(y) #loop through feature sets
  {
    ...
    CV_list <- lapply(fold.list, function(k) #loop through folds
    {
      ...
      score_df <- as.data.frame(kNN_k_acc(train_fold, valid_fold, kNN_parameters))
      return (score_df) #return the mean accuracy of 10 folds in each parameter
    }) # multicore processing

    ave_CV_df <- cbind(kNN_parameters, 'AUROC' = as.numeric(rowMeans(as.data.frame(CV_list)))) #add HPs to the df
...
mat_cv_list <- foreach(i = 1:length(mat_names), .packages = c("class", "pROC", "caret")) %dopar% {MAT_CV(mat_names[i])}
