...
os.chdir("Y:/data/Feature_selection_framework/5CV_sep/")
...
def main(x):
    ...
    filter_file = glob.glob('../4feature_selection_sep/filter_wilc/' + mat_name + '/*on_vol.csv', recursive=True) #get filter features
    filter_list = pandas.read_csv(filter_file[0])['feature']

    embedded_file = glob.glob('../4feature_selection_sep/embedded_wilc/' + mat_name + '/*on_sigVar_pass_list.csv', recursive=True) #get the embedded features
...
on_files = glob.glob("../3transformed_data_sep/*/*on_train.csv")
...
if __name__ == '__main__':
    ...
    MAT_auc_df.to_csv(out_path + "MLP_" + f_name + '_CV.csv', sep=',', index = False)
