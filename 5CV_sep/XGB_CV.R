...
MAT_CV <- function(i) # i = matricization method name
{
  ...
  ave_cv_feature.list <- lapply(feature_set, function(y) #loop through feature sets
  {
    ...
    CV_list <- lapply(fold.list, function(k) #loop through folds
    {
      ...
      #encode the labels
      levels(train_fold$uid) <- c(0,1) #1 is WT, 0 is Q344X
      train_fold$uid <- as.numeric(as.character(train_fold$uid))

      valid_fold$uid <- sapply(strsplit(valid_fold$uid, "_"), '[', 3)
      valid_fold$uid <- as.factor(as.numeric(factor(valid_fold$uid)))
      #encode the labels
      levels(valid_fold$uid) <- c(0,1) #1 is WT, 0 is Q344X
      valid_fold$uid <- as.numeric(as.character(valid_fold$uid))
