...
training_list <- pblapply(data_files, Prep_LightNorm)
...
saveRDS(training_list, "int_output/light_normalized_List_optmized_all_lightOff.Rds")

## batch normalization ##
training_list <-readRDS("int_output/light_normalized_List_optmized_all_lightOff.Rds")
...
saveRDS(training_df.norm, "int_output/batch_corrected_optimized_all_lightOff.Rds")
rm(training_df)
...
for(i in 1:length(treatment_list))
{
  ...
  genorm <- pbapply(treatment_list[[i]][,6:14], 1, function(x) x- dk_adpt.prd.df[i,]+mub)
  #transform the list to a data frame
...
saveRDS(Training_bl_df, "int_output/blNorm_opt_all_lightOff.Rds")
