...
NB_k_acc <- function(train, valid, par_list)
{
  X <- as.matrix(train[,-1])
  Y <- train[,1]
...
{
  input <- input_list[[i]] #GLOBAL VAR: on_list
  ...
  per_feature.list <- lapply(names(feature_set), function(y)
  ## cross validation lapply(names(feature_set), function(y) #loop through feature sets
  {
    ...
    #get the best parameter set for the feature set
    NB_parameters <- cv_out_sub[cv_out_sub$feature == y ,c(5,6)]#conver to logical
    NB_parameters$usekernel <- ifelse(NB_parameters$usekernel == TRUE, TRUE, FALSE)

    #calculate the CV accuracy
    score_df <- as.data.frame(NB_k_acc(train_fold, valid_fold, NB_parameters)) #return the performance in each parameter
...
off_list <- lapply(off_files, read.csv)
