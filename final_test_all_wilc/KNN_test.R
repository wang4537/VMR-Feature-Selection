...
MAT_CV <- function(i) # i = matricization method name
{
  ...
  per_feature.list <- lapply(names(feature_set), function(y)
  {
    ...
    KNN_parameters <- cv_out_sub[cv_out_sub$feature == y , 5]

    #calculate the CV accuracy
    score_df <- as.data.frame(kNN_k_acc(train_fold, valid_fold, KNN_parameters)) #return the performance in each parameter
