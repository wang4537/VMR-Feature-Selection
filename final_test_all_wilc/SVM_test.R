...
MAT_CV_SVM <- function(i, kernel_name, CV_OUT) # i = matricization method name; kernel_name = kernel for SMV
{
  ...
  cv_out_sub <- CV_OUT[CV_OUT$mat == i,]
  ...
  if(kernel_name == 'linear')
  {
    per_feature.list <- lapply(names(feature_set), function(y) #loop through feature sets
    {
      ...
      #hyperparameters
      SVM_parameters <- cv_out_sub[cv_out_sub$feature == y , 5]
      #calculate the CV accuracy
      score_df <- as.data.frame(SVM_linear_acc(train_fold, valid_fold, SVM_parameters))
  ...
  if(kernel_name == 'polynomial')
  {
    per_feature.list <- lapply(names(feature_set), function(y) #loop through feature sets
    {
      ...
      valid_fold <- input[out_test,]
      #hyperparameters
      SVM_parameters <- cv_out_sub[cv_out_sub$feature == y , 5:8]
      #calculate the CV accuracy
      score_df <- as.data.frame(SVM_poly_acc(train_fold, valid_fold, SVM_parameters))
      return (score_df) #return the mean accuracy of 10 folds in each parameter
    })
    names(per_feature.list) <- names(feature_set)#rename the list
    per_feature.df <- do.call("rbind", per_feature.list) #bind the dfs by rows
  ...
  if(kernel_name == "radial")
  {
    per_feature.list <- lapply(names(feature_set), function(y) #loop through feature sets
    {
      ...
      valid_fold <- input[out_test,]
      #hyperparameters
      SVM_parameters <- cv_out_sub[cv_out_sub$feature == y , 5:6]
      #calculate the CV accuracy
      score_df <- as.data.frame(SVM_rad_acc(train_fold, valid_fold, SVM_parameters))
      return (score_df) #return the mean accuracy of 10 folds in each parameter
    })
  ...
  if(kernel_name == "sigmoid")
  {
    per_feature.list <- lapply(names(feature_set), function(y) #loop through feature sets
    {
      ...
      valid_fold <- input[out_test,]
      #hyperparameters
      SVM_parameters <- cv_out_sub[cv_out_sub$feature == y , 5:7]
      #calculate the CV accuracy
      score_df <- as.data.frame(SVM_sig_acc(train_fold, valid_fold, SVM_parameters))
      return (score_df) #return the mean accuracy of 10 folds in each parameter
    })
...
SVM_CV.list <- lapply(SVM_names, function(s)
{
  ...
  cv_out_file <- paste("../CV_wilc/", class_name, "/", class_name, "_", s, "_", f_name, "_CV.csv", sep = "")
  cv_out <- read.csv(cv_out_file)

  mat_cv_list <- lapply(mat_names, function(n) MAT_CV_SVM(n,s,cv_out))
  ...
  #save the CV result
  out_file <- paste("./SVM/SVM_", s, "_", f_name, "_test_wilc.csv",sep = '')
