...
library(rstatix)
library(dplyr)
library(doParallel) # multi-core processing

setwd("Y:/data/Feature_selection_framework/4feature_selection_sep/filter_wilc/")
#light on
...
on_list.fs <- foreach(i = 1:length(on_list), .packages = c("dplyr", "rstatix")) %dopar% {t_test_fs(on_list[[i]])}
...
mat_names <- gsub("../../3transformed_data_sep/|/.*$", "", off_files) #keep only the mid dir name
off_list <- lapply(off_files, read.csv) #read the files
names(off_list) <- mat_names #rename the list

cl <- makeCluster(3)#doParallel, setup parallel processing
registerDoParallel(cl)
off_list.fs <- foreach(i = 1:length(off_list), .packages = c("dplyr", "rstatix")) %dopar% {t_test_fs(off_list[[i]])}
